// Geometric Shape Components for Ingress Traffic Flow Animation
...
export function Cloud({ size = 60, color, isActive, isHighlighted, label, onClick }: ShapeProps) {
  ...
  return (
    ...
    >
      <svg width={width} height={height} style={{ filter: glowFilter(color, !!isActive) }}>
        <path
          d={`M${width*0.25},${height*0.7} 
              a${height*0.25},${height*0.25} 0 0,1 0,-${height*0.35}
              a${height*0.3},${height*0.3} 0 0,1 ${width*0.25},-${height*0.15}
              a${height*0.35},${height*0.35} 0 0,1 ${width*0.35},${height*0.1}
              a${height*0.25},${height*0.25} 0 0,1 ${width*0.05},${height*0.4}
              z`}
          fill={isActive ? color : `${color}cc`}
          stroke={color}
          strokeWidth={isActive ? 3 : 2}
        ...
        {label && (
          <text x={width/2} y={height/2} textAnchor="middle" dominantBaseline="middle" 
                fill="white" fontSize={height/5} fontWeight="bold">
            {label}
          </text>
        )}
      </svg>
