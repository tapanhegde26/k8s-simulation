// Animated Connection Line for Node-Control Plane Communication Flow
...
export function ConnectionLine({ 
  ...
  const packetColor = commColors[commType] || color;
  ...
  return (
    ...
    >
      ...
      <line
        ...
        strokeDasharray={commType === 'tunnel' && isActive ? '4,4' : 'none'}
      ...
      {isActive && (
        <>
          ...
          {/* Lock icon for secure/tunnel traffic */}
          {(commType === 'secure' || commType === 'node-to-cp') && (
            <motion.text
              fill={packetColor}
              fontSize="10"
              textAnchor="middle"
              dominantBaseline="middle"
              initial={{ x: from.x, y: from.y - 16, opacity: 0 }}
              animate={{
                x: [from.x, to.x],
                y: [from.y - 16, to.y - 16],
                opacity: [0, 1, 1, 0],
